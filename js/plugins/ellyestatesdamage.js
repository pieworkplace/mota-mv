    ...
    Game_Battler.prototype.ProcessEllyeStatesDamage = function() {
        this.states().forEach(function(state) {
            if (typeof state.meta !== 'undefined' && state.meta !== null && typeof state.meta.formula !== 'undefined')
            {
                ...
                //junlin added: add status before battle will crash, so add this line
                if (!BattleManager._statusWindow) return;
                ...
                if (this.isActor())
                {
                    let actorSprites = BattleManager._spriteset._actorSprites
                    actorSprites.forEach(function(sprite)
                    {
